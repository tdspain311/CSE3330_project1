...
while(!feof($myfile)){
    ...
    $newstring = preg_replace("/[\r]/","",$line);
 
    list($Country_Name, $Population, $No_of_Worldcup_won, $Manager)=explode(",", $newstring);

    $stmt = $conn->prepare("INSERT INTO `soccer`.`country` (`Country_Name`, `Population`, `No_of_Worldcup_won`, `Manager`) VALUES (?,?,?,?)");
 
    $stmt->bind_param("sdis", $Country_Name, $Population, $No_of_Worldcup_won, $Manager);
    $stmt->execute();
...
//  
// 
//    list($Player_id, $Name, $FName, $LName, $DOB, $Country, $Height, $Club, $Position, $Caps_for_Country, $Is_captain)=explode(",", $newstring);
//
...
// 
//    $stmt->bind_param("isssssissis", $Player_id,$Name,$FName,$LName,$DOB,$Country,$Height,$Club,$Position,$Caps_for_Country,$Is_captain);
//    $stmt->execute();
//    
//    $stmt->close();
//    
...
while(!feof($myfile)){
    ...
    list($Match_id, $Date_of_Match, $Start_Time_Of_Match, $Team1, $Team2, $Team1_score, $Team2_score, $Stadium_Name, $Host_City)=explode(",", $newstring);
    ...
    $stmt->bind_param("issssiiss", $Match_id, $Date_of_Match, $Start_Time_Of_Match, $Team1, $Team2, $Team1_score, $Team2_score, $Stadium_Name, $Host_City);
    $stmt->execute();
...
while(!feof($myfile)){
    ...
    list($Player_id, $No_of_Matches, $Goals, $Assists, $Minutes_Played)=explode(",", $newstring);

  
 $stmt = $conn->prepare("INSERT INTO `soccer`.`player_assists_goals` (`Player_id`, `No_of_Matches`, `Goals`, `Assists`, 'Minutes_Played') VALUES (?,?,?,?,?)");
 

    $stmt->bind_param("iiiii", $Player_id, $No_of_Matches, $Goals, $Assists, $Minutes_Played);
    $stmt->execute();
...
while(!feof($myfile)){
    ...
    $newstring = preg_replace("/[\r]/","",$line);
    	...
    	list($Player_id, $Yellow_Cards, $Red_Cards)=explode(",", $newstring);

    $stmt = $conn->prepare("INSERT INTO `soccer`.`player_cards` (`Player_id`, `Yellow_Cards`,  `Red_Cards`) VALUES (?,?,?)");
 
    $stmt->bind_param("iii", $Player_id, $Yellow_Cards, $Red_Cards);
    $stmt->execute();
